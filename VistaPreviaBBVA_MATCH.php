<?php
    ...
    if ($identioficador !== '') {
        ...
        while ($row = mysqli_fetch_array($queryVISTAPREV)) {
            if ($row['TARJETA'] != 'TARJETABBVA' && $row['TABLE_MATCH'] == 'si') {
                continue;
            }
            ...
            $checkboxState = $match->validaexistematch($idd, $identioficador, 'TARJETABBVA');
            $checkboxDisabled = $match->validaexistematch3res($idd, $identioficador, $permiso, 'TARJETABBVA', $var_respuesta);
            $checkboxAttributes = trim($checkboxState . ' ' . $checkboxDisabled);
            $matchBadge = $match->validaexistematch4($idd, 'TARJETABBVA');
            ...
            $rowsHtml[] = '<tr data-match-status="' . $matchStatus . '">' .
                ...
                '<td class="match-col-match text-nowrap">' .
                    '<input type="checkbox" style="width:30px; color:red;" name="documentos' . $idd . '" value="' . $idd . '" class="form-check-input" id="' . $checkboxId . '" onclick="pasarmatchdocumento(' . $idd . ',' . (int) $identioficador . ',\'TARJETABBVA\')" ' . $checkboxAttributes . '> ' .
