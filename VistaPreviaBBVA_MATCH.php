...
//VISTA PREVIA PARA REALIZAR EL MATCH
      ...
      <div class="table-responsive">  
		   ...
		   ';
    		...
    		while($row = mysqli_fetch_array($queryVISTAPREV)){
				if($row['TARJETA']!='TARJETABBVA' and $row['TABLE_MATCH']=='si'){
...
<td width="20%">$'. number_format( $row["total"],2,'.',',').'</td>
			...
			onclick="pasarmatchdocumento('.$row["idd"].','.$identioficador.',\'TARJETABBVA\')" 
			'./*regresa checked*/$match->validaexistematch($row["idd"],$identioficador,'TARJETABBVA').' 
			'./*regresa disabled*/$match->validaexistematch3res($row["idd"],$identioficador,$permiso,'TARJETABBVA',$var_respuesta).'>

			&nbsp;&nbsp;&nbsp;&nbsp;'.$match->validaexistematch4($row["idd"],'TARJETABBVA').'
			</td>
</tr> 
