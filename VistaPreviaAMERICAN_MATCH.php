<?php
    ...
    if ($identioficador !== '') {
        ...
        while ($row = mysqli_fetch_array($queryVISTAPREV)) {
            if ($row['TARJETA'] != 'AMERICANE' && $row['TABLE_MATCH'] == 'si') {
                continue;
            }
            ...
            $checkboxState = $match->validaexistematch($idd, $identioficador, 'AMERICANE');
            $checkboxDisabled = $match->validaexistematch3res($idd, $identioficador, $permiso, 'AMERICANE', $var_respuesta);
            $checkboxAttributes = trim($checkboxState . ' ' . $checkboxDisabled);
            $matchBadge = $match->validaexistematch4($idd, 'AMERICANE');
            ...
            $rowsHtml[] = '<tr data-match-status="' . $matchStatus . '">' .
                ...
                '<td class="match-col-match text-nowrap">' .
                    '<input type="checkbox" style="width:30px; color:red;" name="documentos' . $idd . '" value="' . $idd . '" class="form-check-input" id="' . $checkboxId . '" onclick="pasarmatchdocumento(' . $idd . ',' . (int) $identioficador . ',\'AMERICANE\')" ' . $checkboxAttributes . '> ' .
